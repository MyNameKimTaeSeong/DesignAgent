...
<script type="text/javascript">
	...
	function heartClick(k, id, p_num){
		if(${id == null} ){
			alert("로그인 후 이용해주세요!");
		}else{
	...
	function popup(){
		...
		window.open("../img/mass.jpg", wName, options);
	}
</script>
</head>
<body onload="popup()">
	...
	<div id="wrapper">
		...
		<div id="content" class="boxSize">
			...
			<div id="menu_line" class="boxSize">
				<span>Home > Clothes Draft > 대량생산</span>
			</div>
			<!-- Best -->
			...
			<div id="category_btn" class="boxSize">
				<a href="../Draft/Select_Draft.do?sort=m&category=상의&nowPage=1"><input type="button" class="category_css cate_btn" value="상의"></a>
				<a href="../Draft/Select_Draft.do?sort=m&category=하의&nowPage=1"><input type="button" class="category_css cate_btn" value="하의"></a>
				<a href="../Draft/Select_Draft.do?sort=m&category=원피스&nowPage=1"><input type="button" class="category_css cate_btn" value="원피스"></a>
				<a href="../Draft/Select_Draft.do?sort=m&category=아우터&nowPage=1"><input type="button" class="cate_btn" value="아우터"></a>
			</div>
			...
			<div id="division" class="boxSize">
				...
				<c:if test="${division == null}">
					<a href="../Draft/Select_Draft.do?sort=m&nowPage=1&division=newest" style="font-weight:bold" >최신순</a>
					<a href="../Draft/Select_Draft.do?sort=m&nowPage=1&division=popularity">인기순</a>
				</c:if>
				<c:if test="${division != null}">
					<c:if test="${division == 'popularity'}">
						<a href="../Draft/Select_Draft.do?sort=m&nowPage=${nowPage}&division=newest">최신순</a>
						<a href="../Draft/Select_Draft.do?sort=m&nowPage=${nowPage}&division=popularity" style="font-weight:bold">인기순</a>
					</c:if>
					<c:if test="${division == 'newest'}">
						<a href="../Draft/Select_Draft.do?sort=m&nowPage=${nowPage}&division=newest" style="font-weight:bold">최신순</a>
						<a href="../Draft/Select_Draft.do?sort=m&nowPage=${nowPage}&division=popularity">인기순</a>
					</c:if>
				</c:if>				
			</div>
			...
			<div id="draft_container" class="boxSize">
				...
				<c:if test="${draftList != null}">
					<c:forEach var="list" items="${draftList}" varStatus="v">
						...
						<!-- 4열 사진 -->
						<c:if test="${v.count % 4 == 0}">
							<div id="draft boxSize" class= "boxSize">
								<!-- draft 사진. click하면 detail 창-->
								<div id="draft_img" class= "boxSize">
									<img alt="" src="${list.photo_url}" class="img" onclick="draftDetail(${list.p_num})">
								</div>
			...
			<div id="page_btn" class="boxSize">
				<!-- 이전 step으로 이동 -->
				<a href="../Draft/Select_Draft.do?sort=m&nowPage=${prePage}&division=${division}"><span class="page_style paging">&laquo;</span></a>&nbsp;
				<!-- 페이징 -->
				<c:forEach var="i" begin="${start_page}" end="${end_page}" step="1">
					<c:if test="${i == nowPage}">
						<a class="page_num" href="../Draft/Select_Draft.do?sort=m&nowPage=${i}&division=${division}"><span class="page_style paging" style="color:#b22222">${i}</span></a>&nbsp;
					</c:if>
					<c:if test="${i != nowPage}">
						<a class="page_num" href="../Draft/Select_Draft.do?sort=m&nowPage=${i}&division=${division}"><span class="page_style paging">${i}</span></a>&nbsp;
					</c:if>
				</c:forEach>
				<!-- 다음 step으로 이동 -->
				<a href="../Draft/Select_Draft.do?sort=m&nowPage=${nextPage}&division=${division}"><span class="page_style paging">&raquo;</span></a>&nbsp;
				
				<!-- upload 버튼 -->
				<a href="draft_mass_upload.jsp" id="upload"><input type="button" value="Upload" id="upload_btn"></a>
			</div>		
		</div>
		<!-- footer -->
